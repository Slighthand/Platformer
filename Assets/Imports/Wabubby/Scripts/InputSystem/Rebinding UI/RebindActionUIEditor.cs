    ...
    public class RebindActionUIEditor : UnityEditor.Editor
    {
        ...
        protected void RefreshBindingOptions()
        {
            ...
            for (var i = 0; i < bindingCount; ++i)
            {
                ...
                // If we don't have a binding groups (physics schemes), show the device that if there are, for example,
                ...
                // If the binding is part of physics schemes, mention them.
                if (haveBindingGroups)
                {
                    var asset = action.actionMap?.asset;
                    if (asset != null)
                    {
                        var physicsSchemes = string.Join(", ",
                            binding.groups.Split(InputBinding.Separator)
                                .Select(x => asset.controlSchemes.FirstOrDefault(c => c.bindingGroup == x).name));

                        displayString = $"{displayString} ({physicsSchemes})";
                    }
                }
