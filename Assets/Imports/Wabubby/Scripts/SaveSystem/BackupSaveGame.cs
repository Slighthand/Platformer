...
public class BackupSaveGame : SaveGame
{
    ...
    public void ShuffleSave(bool isEncrypted) {
        ...
        string backupNarnia = Combine(SaveSystem.OldSavesDirectoryPath, BackupSlot.Name);
    ...
    public void Load(bool isEncrypted) {
        try {
            ...
            Debug.LogError($"slot at game {Name} failed to load. your backup savefile has been loaded instead. you can find the [UNPLAYABLE] slot in /old-saves. literally unplayable man.");
    ...
    public void LoadBackup(bool isEncrypted) {
        if (BackupSlot == null) {
            Debug.Log("you done messed man. there are no backups no more. get me a pizariella");
            return;
        }

        File.Move(CurSlot.Path, Combine(SaveSystem.OldSavesDirectoryPath,BackupSlot.Name)); // this will be replaced with an ACTUAL deletion later
        CurSlot = BackupSlot;
        BackupSlot = null;
        Load(isEncrypted);
